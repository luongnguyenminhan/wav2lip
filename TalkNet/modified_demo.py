...
def visualization(tracks, scores, args):
    ...
    for fidx, fname in tqdm.tqdm(enumerate(flist), total = len(flist)):
        ...
        for face in faces[fidx]:
            ...
            if txt >= 0:
                # Step 1: create a loop to define the list of duration has talking face --> list in list
                
                # Step 2: crop video (include audio and frame) following the output of step 1 (represent for frames having talking face)
                ...
                #     y = int(face['y'] - face['s'])
                #     s = int(face['s'])
                #     print(x, y, s)# Adjust the bounding box coordinates to fit within the image bounds
                #     x = max(x, 0)
                #     y = max(y, 0)
                #     s = min(s, min(image.shape[0], image.shape[1]))

                #     # Crop the face from the image
                #     cropped_face = image[y:y + s, x:x + s]
                #     # print(cropped_face)
                #     # Save the cropped face to a new file
                #     output_file = "%09d.jpg"%fidx
                    # cv2.imwrite(output_file, cropped_face)
                # draw bouding box for speaker
                # (x,y) (x+w, y+h)
                cv2.rectangle(image, (int(face['x']-face['s']), int(face['y']-face['s'])), (int(face['x']+face['s']), int(face['y']+face['s'])),(0,clr,255-clr),10)
                print((int(face['x']-face['s']), int(face['y']-face['s'])), (int(face['x']+face['s']), int(face['y']+face['s'])))
                # show metric measurement the detect which person is currently speaking
                cv2.putText(image,'%s'%(txt), (int(face['x']-face['s']), int(face['y']-face['s'])), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,clr,255-clr),5)
                vOut.write(image)
	# vOut now have full video with bouding box
    vOut.release()
...
def get_duration_list(tracks, scores):
    # CPU: visulize the result for video format
    durations_list = []
    for tidx, track in enumerate(tracks):
        score = scores[tidx]
        ...
        temp = []
        start = max(fidx - 2, 0)
        end = min(fidx + 3, len(score) - 1)
        length = len(track['track']['frame'].tolist())
        while fidx < length:
            new_start = max(fidx - 2, 0)
            new_end = min(fidx + 3, len(score) - 1)
            s = score[new_start:new_end] # average smoothing
            s = numpy.mean(s)
            # print(f"\ttdix: {tidx}, fidx: {max(fidx - 2, 0)} to {min(fidx + 3, len(score) - 1)}, s: {s}")
            if s >= 0:
                duration = (start, new_end)
                temp.append(duration)
            else:
                if len(temp) > 0:
                    final.append(temp[-1])
                temp = []
                duration = (new_start, new_end)
                start = max(fidx + 1 - 2, 0)
            fidx+=1
        if len(temp) > 0:
            final.append(temp[-1])
            
        print(final)
        durations_list.append(final)
    return durations_list

def crop(args, track, durations, cropFile):
    # Step 1: create a loop to define the list of duration has talking face --> list in list
    # Step 2: crop video (include audio and frame)
    
    # following the output of step 1 (represent for frames having talking face) 
    # CPU: crop the face clips
    flist = glob.glob(os.path.join(args.pyframesPath, '*.jpg')) # Read the frames
    flist.sort()
    vOut = cv2.VideoWriter(cropFile + 't.avi', cv2.VideoWriter_fourcc(*'XVID'), 25, (224,224))# Write video
    ...
    for duration in durations:
        ...
        for fidx, frame in enumerate(track['frame'][a:b+1]):
            cs  = args.cropScale
            bs  = dets['s'][fidx]   # Detection box size
            bsi = int(bs * (1 + 2 * cs))  # Pad videos by this amount 
            image = cv2.imread(flist[frame])
            ...
            vOut.write(cv2.resize(face, (224, 224)))
        audioTmp    = cropFile + '.wav'
        audioStart  = (track['frame'][a]) / 25
        audioEnd    = (track['frame'][b]+1) / 25
        vOut.release()
    command = ("ffmpeg -y -i %s -async 1 -ac 1 -vn -acodec pcm_s16le -ar 16000 -threads %d -ss %.3f -to %.3f %s -loglevel panic" % \
              (args.audioFilePath, args.nDataLoaderThread, audioStart, audioEnd, audioTmp)) 
    output = subprocess.call(command, shell=True, stdout=None) # Crop audio file
...
def main():
    ...
    print(scores)
    duration_list = get_duration_list(vidTracks, scores)
    for ii, durations in enumerate(duration_list):
        crop(args, track, durations, os.path.join(args.pycropPath, '%05d'%ii))
